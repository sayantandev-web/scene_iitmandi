	...
	public function add_team($id='') {
		...
		if($this->input->post()) {
			...
			if(!empty($id)) {
				...
				$check_email = $this->db->query("SELECT *  FROM `iitmandi_team` WHERE `email` = '".$this->input->post('email')."' and `is_delete` = 1");
		...
		$data['specialization']=$this->common_model->get_data_array(SPECIALIZATION,'','','','','','',SPECIALIZATION.".id DESC",array('status'=>1, 'is_delete'=>1));
